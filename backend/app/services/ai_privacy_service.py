...
def mask_ai_sensitive_text(text: str, settings: AISettings | None = None) -> str:
    ...
    if mask_phones:
        value = re.sub(r"(?<!\d)(?:\+?\d[\d\s-]{7,}\d)(?!\d)", "***MASKED_PHONE***", value)
    if mask_employee_ids:
        value = re.sub(r"(?i)\b(?:employee\s*id|emp\s*id|الرقم\s*الوظيفي)\s*[:=]?\s*[A-Za-z0-9_-]{2,}\b", "***MASKED_EMPLOYEE_ID***", value)
    if mask_usernames:
        value = re.sub(r"(?i)\b(?:username|user|اسم\s*المستخدم)\s*[:=]?\s*[A-Za-z0-9_.-]{2,}\b", "***MASKED_USERNAME***", value)
    if mask_requests:
