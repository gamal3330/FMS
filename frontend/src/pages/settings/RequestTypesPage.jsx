  ...
  async function load() {
    try {
      const [typesResponse, departmentsResponse] = await Promise.all([
        api.get("/request-types", { params: { search: search || undefined, status: status || undefined } }),
        api.get("/departments")
      ]);
      setItems(typesResponse.data);
      setDepartments(departmentsResponse.data);
      setSelected((current) => current || typesResponse.data[0] || null);
    } catch (error) {
      ...
      {modal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/70 p-4 backdrop-blur-sm">
          <div className="max-h-[90vh] w-full max-w-4xl overflow-y-auto rounded-lg bg-white p-5 shadow-xl">
            ...
            <RequestTypeForm value={modal === "edit" ? selected : null} onSubmit={saveType} onCancel={() => setModal(false)} />
          </div>
        </div>
      )}
    </section>
