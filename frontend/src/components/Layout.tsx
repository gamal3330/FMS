import { Activity, BarChart3, Bell, Building2, FileText, KeyRound, LayoutDashboard, LogOut, Network, ScrollText, Settings, ShieldCheck, UserCircle, Users, X } from "lucide-react";
...
export function Layout({
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-950" dir="rtl">
      ...
      <aside className="fixed right-0 top-0 hidden h-full w-72 border-l border-slate-200 bg-white p-5 lg:block">
        <div className="mb-8">
          {logoUrl ? (
        ...
        <nav className="space-y-2">
          {nav.map(({ label, href, icon: Icon }) => (
            <NavLink
              ...
              to={href}
              className={({ isActive }) =>
                `flex h-11 w-full items-center gap-3 rounded-md px-3 text-right text-sm font-medium transition ${
        ...
        <div className="absolute bottom-5 left-5 right-5">
          ...
          <button onClick={() => setPasswordDialogOpen(true)} className="mb-2 flex w-full items-center gap-2 rounded-md px-3 py-2 text-sm text-slate-600 hover:bg-bank-50 hover:text-bank-700">
            <KeyRound className="h-4 w-4" />
            تغيير كلمة المرور
          </button>
          <button onClick={onLogout} className="flex w-full items-center gap-2 rounded-md px-3 py-2 text-sm text-slate-500 hover:bg-red-50 hover:text-red-700">
