...
- دعم المراحل الشرطية المرتبطة بقيم حقول الطلب؛ المرحلة غير المنطبقة تحفظ داخل النسخة بحالة `skipped` ولا تعطل المسار.
- حفظ تاريخ إجراءات الموافقات في `request_approval_actions` مع رقم مراجعة المسار حتى لا تضيع الإجراءات السابقة بعد الإرجاع وإعادة الإرسال.
- استئناف المسار بعد إعادة الإرسال من المرحلة المحددة في `return_to_step_order`، أو من أول مرحلة عند عدم تحديدها.
- تشغيل مهمة Quartz دورية لتصعيد مرحلة الموافقة عند تجاوز `SlaDueAt` إلى المستخدم أو الدور المحدد، مع إشعار فوري وسجل تدقيق.

هذه المرحلة تعتمد على `request_workflow_snapshots` ولا تستخدم قالب المسار الحي عند معالجة طلب قائم.

يمكن ضبط تكرار فحص التصعيد من الإعداد:

```json
{
  "Workflow": {
    "SlaEscalationIntervalMinutes": 1
  }
}
```
